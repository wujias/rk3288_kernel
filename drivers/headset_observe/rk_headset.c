...
struct headset_priv *headset_info;
EXPORT_SYMBOL(headset_info);
...
static void headsetobserve_work(struct work_struct *work)
{
	...
	{
		if (gpio_is_valid(pdata->phone_ctl_gpio))
			gpio_set_value(pdata->phone_ctl_gpio, 1);
	...
	{	
		if (gpio_is_valid(pdata->phone_ctl_gpio))
			gpio_set_value(pdata->phone_ctl_gpio, 0);
