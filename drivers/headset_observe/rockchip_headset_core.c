...
static int rockchip_headset_probe(struct platform_device *pdev)
{
	...
	ret = of_get_named_gpio_flags(node, "phone_ctl_gpio", 0, &flags);
	if (ret < 0) {
		printk("%s() Can not read property phone_ctl_gpio\n", __FUNCTION__);
		goto err;
	} else {
		pdata->phone_ctl_gpio = ret;
		ret = devm_gpio_request(&pdev->dev, pdata->phone_ctl_gpio, "phone_ctl_gpio");
		if(ret < 0){
			printk("%s() devm_gpio_request phone_ctl_gpio request ERROR\n", __FUNCTION__);
			goto err;
		}
		ret = gpio_direction_output(pdata->phone_ctl_gpio,0); 
	}
